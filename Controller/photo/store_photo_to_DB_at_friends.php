...
//next, we insert into marked_photos table
...
{ 
    $sql="insert into marked_photo (marker_user_id,marker_username,be_marked_user_id,be_marked_nickname,
	         photo_path,group_hobby,show_to_fans,date,marked_description,photo_id) values ('".$_SESSION['user_id']."','".$_SESSION['login_name']."',
	         '".substr($user_id_string,0,strlen($user_id_string)-1)."','".substr($nick_name_string,0,strlen($nick_name_array)-1)."','".$_POST['photo_name'][$i]."',
	         '".$_POST['postToTagAtFriend']."','".$_POST['postToAllAtFriend']."',
	         '".date("Y-m-d h:m:s")."','".$_POST['marked_description']."','".$inserted_photos_id_array[$i]."')";
	mysql_query($sql);
}
$sql_insert_to_friend_news="insert into friends_movement (marker_user_id,marker_nick_name,
		                        marker_login_name,be_marked_user_id,photo_path,date,photo_uploaded_amount,all_photo_id) values ('".$user_id."',
								'".$_SESSION['nick_name']."','".$_SESSION['login_name']."','".substr($user_id_string,0,strlen($user_id_string)-1)."',
								'".$one_of_photo_path."','".date("Y-m-d h:m:s")."','".$photo_uploaded_amount."','".$all_photo_id."')";
