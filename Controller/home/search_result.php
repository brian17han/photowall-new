...
while($out=@mysql_fetch_array($result_friends["ResultSet1"]))
{
	...
	$temp=array("nick_name"=>$out["nick_name"],"photo_path"=>$head_photo_folder.$out["user_id"]."_".$out["head_photo"],
	"location"=>$out["province"].",".$out["city"],"introduction"=>$out["introduction"],
	"user_id"=>$out["user_id"],"flag"=>"我的贴友","concerns_count"=>$arr[0],"fans_count"=>$arr[1],"friends_count"=>$arr[2]);
...
while($out=@mysql_fetch_array($result_concerns["ResultSet"]))
{
	...
	$temp=array("nick_name"=>$out["nick_name"],"photo_path"=>$head_photo_folder.$out["user_id"]."_".$out["head_photo"],
	"location"=>$out["province"].",".$out["city"],"introduction"=>$out["introduction"],
	"user_id"=>$out["user_id"],"flag"=>"我的关注","concerns_count"=>$arr[0],"fans_count"=>$arr[1],"friends_count"=>$arr[2]);
	array_push($Final_ResultSet,$temp);
}
while($out=@mysql_fetch_array($result_fans["ResultSet"]))
{
	...
	$temp=array("nick_name"=>$out["nick_name"],"photo_path"=>$head_photo_folder.$out["user_id"]."_".$out["head_photo"],
	"location"=>$out["province"].",".$out["city"],"introduction"=>$out["introduction"],
	"user_id"=>$out["user_id"],"flag"=>"我的粉丝","concerns_count"=>$arr[0],"fans_count"=>$arr[1],"friends_count"=>$arr[2]);
	array_push($Final_ResultSet,$temp);
}
